...
class TeamScraper:
    ...
    def mark_success(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            f"UPDATE public.teams SET scraped='true' WHERE team_id = '{self.team_id}';"
        )
        conn.commit()
    ...
    def go(self):
        ...
        while True:
            ...
            try:
                ...
                df.to_sql("dota_matches", engine, if_exists="append", index=False)
                ...
                print(
                    f"Last  url failed: {url} time: {time.strftime('%Y-%m-%d %H:%M')}"
                )
                traceback.print_exc()
                exit()
        self.mark_success()
