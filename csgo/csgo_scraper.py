...
from csgo.csgo_match import CSGOMatch
    ...
    with requests.session() as session:
        ...
        while True:
            try:
                ...
                results_soup = bs.BeautifulSoup(
                    session.get(results_url, headers=headers).text, "html5lib"
                )

                if (
                    results_soup.find("h1", {"data-translate": "block_headline"})
                    is not None
                ):
                    raise Blocked

                for match in results_soup.find_all("div", {"class": "result-con"}):
...
if __name__ == "__main__":
    ...
    pending_urls = scrape_urls[
        (scrape_urls["scraped"] != False) & (scrape_urls["scraped"] != True)
    ]
    c = Sess()
    ...
    for index, row in pending_urls.iterrows():
        ...
        if (
            url
            == "https://www.hltv.org/matches/2297915/night-raid-vs-kappakappa-dreamhack-open-cluj-napoca-2015-eu-pre-qualifier-2"
            or url
            == "https://www.hltv.org/matches/2330407/ut-austin-vs-kennesaw-state-university-java-collegiate-2"
        ):
            continue
        try:
        ...
        for game_index, game in enumerate(games):
            ...
            games_df = games_df.append(
                {
                    ...
                    "game_id": game.site_game_id,
                    "match_id": site_match_id,
                    "nth_match": game_n_in_match,
                },
                ignore_index=True,
            )
            ...
            for t in [game.t1, game.t2]:
                ...
                game_teams_df = game_teams_df.append(team_repr, ignore_index=True)
        ...
        try:
            players_stats_df.to_sql(
                "players_stats", con=ENGINE, index=False, if_exists="append"
            )
            match_info.to_sql("matches", con=ENGINE, index=False, if_exists="append")
            games_df.to_sql("games", con=ENGINE, index=False, if_exists="append")
            game_teams_df.to_sql(
                "games_teams", con=ENGINE, index=False, if_exists="append"
            )

            cursor.execute(f"UPDATE scrape_urls SET scraped = TRUE WHERE link='{link}'")
            conn.commit()
        except Exception:
            traceback.print_exc()
            # exit()
