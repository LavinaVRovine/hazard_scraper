...
class Games_parser:
    ...
    def parse_page(self, summary_url, df, sql_session):
        ...
        sql_session.add(
            Game_result(
                match_id=int(winner_row.iloc[0, 0]),
                main_team_id=int(winner_row.iloc[0, 1]),
                competitor_team_id=int(loser_row.iloc[0, 1]),
                score=json.dumps(results),
                main_team_won=True,
            )
        )
        sql_session.add(
            Game_result(
                match_id=int(loser_row.iloc[0, 0]),
                main_team_id=int(loser_row.iloc[0, 1]),
                competitor_team_id=(int(winner_row.iloc[0, 1])),
                score=json.dumps(results),
                main_team_won=False,
            )
        )

        sql_session.query(Team_match).filter(Team_match.match_url == url).update(
            {"scraped": True}
        )
...
def scrape_pending_games():
    ...
    df = pd.read_sql(
        "SELECT team_matches.match_id, team_matches.team_id, team_matches.match_url, teams.name FROM team_matches JOIN teams ON teams.team_id = team_matches.team_id WHERE team_matches.scraped isnull",
        engine,
    )
