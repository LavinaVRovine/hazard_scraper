...
for index, row in df.iterrows():
    try:
        ...
        zaklady_statistiky = zaklady_statistiky.reindex(
            zaklady_statistiky.index.drop(0)
        )
        ...
        zaklady_statistiky["CS per Minute:"] = pd.to_numeric(
            zaklady_statistiky["CS per Minute:"]
        )
        zaklady_statistiky["Gold Per Minute:"] = pd.to_numeric(
            zaklady_statistiky["Gold Per Minute:"]
        )
        ...
        zaklady_statistiky["Kill Participation:"] = zaklady_statistiky[
            "Kill Participation:"
        ].str.replace("%", "")
        zaklady_statistiky["Kill Participation:"] = pd.to_numeric(
            zaklady_statistiky["Kill Participation:"]
        )
        ...
        zaklady_statistiky.to_sql(
            "players_statistics", engine, if_exists="append", index=False
        )
        time.sleep(1)
        if index % 50 == 0:
